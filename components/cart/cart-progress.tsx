        ...
        <div className='flex items-center justify-center pb-6'>
            <div className='w-64 h-3 bg-muted rounded-md relative'>
                <div className='absolute top-0 left-0 h-full w-full flex items-center justify-between'>
                    <motion.span 
                        className='bg-primary h-full rounded-md absolute top-0 left-0 z-30'
                        initial={{ width: 0 }} animate={{ width: checkoutProgress === 'cart-page' ? 0 : checkoutProgress === 'payment-page' ? '50%' : '100%'}}/>
                    <motion.div 
                        initial={{scale: 0}}
                        animate={{scale: checkoutProgress === 'cart-page' ? 1 : 0 || checkoutProgress === 'payment-page' ? 1 : 0 || checkoutProgress === 'confirmation-page' ? 1 : 0}}
                        transition={{ delay: 0.25 }}
                        className='z-50 bg-primary rounded-full p-2'>
